---
name: onboarding
description: Use when a user installs the building-in-the-open plugin for the first time, wants to configure quality gates for their writing, or asks how to set up bito-lint. Conducts a guided interview to discover writing standards and generates a .bito-lint.yaml config.
---
...
Discover a writer's standards, voice, and document types through a guided interview, then generate a `.bito-lint.yaml` config that enforces those standards automatically. The interview replaces the manual work of reading config docs and guessing at thresholds — you answer questions about how you write, and the config writes itself.
...
5. **Present draft config** — show the `.bito-lint.yaml`, iterate until approved
...
- Check for existing `.bito-lint.yaml`, `.bito-lint.toml`, `.bito.yaml`
...
Assemble the `.bito-lint.yaml` from all the answers. Present it to the user:

> Here's your draft config. Take a look — I'll explain any field that's unclear.

```yaml
# .bito-lint.yaml — generated by building-in-the-open onboarding
...
1. **Write `.bito-lint.yaml`** to the project root
...
- **The config is the artifact** — This skill produces a file, not a conversation. The interview is the means, the `.bito-lint.yaml` is the end.
